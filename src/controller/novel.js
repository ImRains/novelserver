 ...
 * @param {*} bookid 
 */
async function getNovelInfoById(bookid) {
    let result = {}
    // 首先查看本地数据库，获取书籍基本信息
    let novelInfo = await getNovelInfoServerById(bookid)
    let _strategy = sources[novelInfo.source].strategy
    ...
    if(!!novelInfo.id){
        ...
        if(chapterList.count == 0){
            // 如果目录列表为空，说明未爬取章节信息
            let sourceNovelChapter = await parser().source(novelInfo.sourceUrl , _strategy.getChapter, _strategy)
            let _list = sourceNovelChapter.chapters
            chapterList.count = _list.length
            chapterList.rows = _list
            // 目录存入数据库
